...
try{
	...
	//GETTING WORLD POST
	$sql_wor_post2=$dbh->prepare('SELECT *FROM world_post WHERE REMOVED="NO" ORDER BY ID DESC LIMIT 3 OFFSET 6');
	$sql_wor_post2->execute();
	
	$num_sql_wor_post2=$sql_wor_post2->rowCount();
	
	if($num_sql_wor_post2>0){
		//require("inc/datetimediff.inc.php");
		while($fetch_sql_wor_post=$sql_wor_post2->fetch(PDO::FETCH_ASSOC)){
		...
		$num_sql_wor=$sql_wor->rowCount();
	 	...
	 	//CHECKING IF THE USER HAS UPLOADED ANY PROFILE PIC
			...
			$world_post2='        <div class="col-md-4 col-sm-4">
