<?php
//CHECKING IF USER IS ALLOWED TO POST IN WORLD

try{
	if($user_login != $world_creator){
		if($world_post=="Only me"){
			
				if($world_comment=="Only me"){
			if($user_login != $world_creator){
			   // header("location:home.php");
			}
		}
		elseif($world_comment=="Friends"){
			if($user_login != $world_creator){
...
elseif($world_comment=="Followers"){
			if($user_login!=$world_creator){
		...
		if($sqlfollower!=1){
			$a_msg="<script type='text/javascript'>
				alert('Sorry Permission Is Need To Comment In World');
			</script>";
			//echo $a_msg;
	//	header("location:home.php");
		}
