...
try{
	...
	if($num_row1>0){
		//echo"cannot skip friend twice sorry";
				...
				$check_friend=$dbh->prepare('SELECT *FROM friends_tbl WHERE SENT_TO="'.$user_frn.'" AND SENT_FROM="'.$user_login.'" AND REMOVED="NO" || SENT_FROM="'.$user_frn.'" AND SENT_TO="'.$user_login.'" AND REMOVED="NO"');
...
try{
	...
	*/
	...
	$users3=$users2;
	$posted_to=$users3;
	$comment='Sent you a friend request';
