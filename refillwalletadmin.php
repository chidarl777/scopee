...
.form-body{
 ...
 <a href="refillwalletadmin.php" >refill wallet</a>
 <a href="updatesmsunit.php"  >Update SMS unit</a>
                          </div>
                      <li>
                          <div class="form-body">
                       ...
                       <div class="input">
                          ...
                          try{
						    ...
						    if(isset($_POST{"send-amt"})){
						  	
                         $error=$amount_paiderr=$bank_usernameerr='';
                          if(!empty($_POST['amount-paid'])){
                          	
			$amount_paid=test_input($_POST['amount-paid']);
			
		}
		else{
			$amount_paiderr='input empty';
		}
		if(!empty($_POST['username'])){
			$bank_username=test_input($_POST['username']);
			
		}
		else{
			$bank_usernameerr='input empty';
		}
		 
		if(empty($error)){
			require('inc/db/config.php');
			
			$sql_unit=$dbh->prepare('SELECT *FROM users WHERE USERNAME="'.$bank_username.'"');
			$sql_unit->execute();
			$num_sql_unit=$sql_unit->rowCount();
		
			if($num_sql_unit>0){
				$fetch_user_unit=$sql_unit->fetch(PDO::FETCH_ASSOC);
				$bank_amt_remaining=$fetch_user_unit['BANK'];
				
				//SELECTING FROM PLAN TABLE
				$sql_rate=$dbh->prepare('SELECT *FROM peecoin_rate WHERE valid="yes"');
				$sql_rate->execute();
				$num_sql_rate=$sql_rate->rowCount();
				if($num_sql_rate==1){
					$fetch_sql_rate=$sql_rate->fetch(PDO::FETCH_ASSOC);
					$peecoin_rate=$fetch_sql_rate['rate'];
					
					
				//INSERTS AMOUNT IN USERS TABLE
				$comfirmed='yes';
				$insert_world_post='INSERT INTO bank(username,amount_paid,peecoin_rate,date_paid,valid) VALUES(:username,:amount_paid,:peecoin_rate,:date,:valid)';
		$STM=$dbh->prepare($insert_world_post);
	$STM->bindParam(":username",$bank_username);
	$STM->bindParam(":amount_paid",$amount_paid);
	$STM->bindParam(":peecoin_rate",$peecoin_rate);
	$STM->bindParam(":date",$date);
	$STM->bindParam(":valid",$comfirmed);
	$STM->execute();
	
	$user_new_bank_amt=$bank_amt_remaining+$amount_paid;
	//UPDATE USERS UNIT
	$unit_update=$dbh->prepare('UPDATE users SET BANK="'.$user_new_bank_amt.'" WHERE USERNAME="'.$bank_username.'"');
	$unit_update->execute();
	echo '<div style="width:90%;font-size:17px; background-color:#dedb54;">Deposited amount was successful</div>';
	echo '<script>alert("Deposited amount was successful");</script>';
	...
	echo("connection error,because:".$error->getMessage());
                       ...
                       <input type="text" class="amt-unit" name="username" placeholder="Enter they username"/><br/><br/>
                       	<input type="text" class="amt-unit" name="amount-paid" placeholder="Enter they amount  to pay"/>
                       </div>
                       <div class="sd-input">
                       	<input type="submit" class="btn-send" name="send-amt" value="Submit"/>
                       </div>
                       </form>
