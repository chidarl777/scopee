...
<head>
  ...
  <title><?php echo $w_name; ?> | Scopee</title>
...
$check_user=$dbh->prepare('SELECT *FROM active_log WHERE USERNAME="'.$user_login1.'" AND ACTIVE="YES" AND INACTIVE="NO"');
	 ...
	 if($get_result>0){
	 	...
	 	$_SESSION['world_adress']=$w_adress;
	...
	//======================================
	//CHECKING IF USER HAS FOLLOWED WORLD
	//======================================
	$aql_follow=$dbh->prepare('SELECT * FROM follow_world WHERE WORLD_ADRESS="'.$w_adress.'" AND WORLD_ID="'.$world_id_w.'" AND FOLLOWED="YES" AND UNFOLLOWED="NO" AND FOLLOWER="'.$user_login.'"');
	$sql_follow->execute();
	$num_sql_follow=$sql_follow->rowCount();
	
	if($num_sql_follow>0){
		//DO NO SHOW THE BUTTON INSTEAD SHOW UNFOLLOW BTN
		$follow_btn=' <button id="follow">Follow</button>';
	}
	else{
		$follow_btn=' <button id="follow">Unfollow</button>'
	}
                              ...
                              <div class="container-fluid">
                                ...
                                <?php
                                 ...
                                 if($user_login != $world_creator){
                                  ...
                                  ?>
                                  <?php require("inc/world/follow_world.inc.php"); ?>
                                ...
                                <a><input type="button" id="change-pic" style=" margin-top:50px;float:left; display:none; " value="Change profile pic"/></a>-->
                                     ...
                                     <div class="push-left" id="change-pic"  style="display:none; ">
	 ...
	 if($world_view=="Only Those Allowed"){
          echo '<div class="unit">
			...
			if($num_sql_acs_code1>0){
				...
				echo '<input style="display:none;" type="text" id="post-permission" value="None" />';
			}
	}
	}
 catch(PDOException $error){
