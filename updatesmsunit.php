...
.form-body{
 ...
 <a href="updatesmsunit.php"  >Update SMS unit</a>
	...
	if(isset($_POST['update-unit'])){
		$error=$sms_unit_usernameerr=$amount_paiderr='';
		if(!empty($_POST['username'])){
			$sms_unit_username=test_input($_POST['username']);
			
		}
		else{
			$sms_unit_usernameerr='field empty<br/><br/>';
			echo $sms_unit_usernameerr;
		}
	...
	//UPDATE USERS UNIT
	$unit_update=$dbh->prepare('UPDATE users SET SMS_UNIT="'.$user_new_unit.'" WHERE USERNAME="'.$sms_unit_username.'"');
	$unit_update->execute();
	
	echo "<script>alert('SMS unit successfully updated');</script>";
	}
			}
			else{
...
<form method="post">
<div class="input">
		<input type="text" class="amt-unit" name="username" placeholder="client username"/><br/><br/>
		<input type="text" class="amt-unit" name="amount-paid" placeholder="users amount paid" /><br/>
		
		</div><br/>
		<input  type="submit" class="btn-send" name="update-unit" value="Update"/>
